...
def predict(model, enc_input, max_dec_len=50):
   model.eval()
   with torch.no_grad():

      enc_input = " ".join(re.findall(r"[\w(')]+", enc_input))
      enc_idx = torch.tensor([[evoc.get(tk, 1) for tk in enc_input.split()]], dtype=torch.long)

      # 编码器
      # h_n, c_n shape:(D * num_layers, batch_size, hidden_size)
      ...
      while True:
         ...
         # 词向量层
         dec_emb = model.decoder_emb(dec_input)
         # rnn层 
         dec_rnn_out, (h_n, c_n) = model.dec_rnn(dec_emb, (h_n, c_n))
         output = model.dec_fc(dec_rnn_out)
         # 得到概率最大的token索引
...
if __name__ == "__main__":
   ...
   # 加载模型
   state_dict = torch.load("model_seq2seq_add.bin", weights_only=False)
   ...
   # =======================================================================
   pred = predict(model, "You've gained weight, haven't you?")
   print("".join(pred))
   # =======================================================================
